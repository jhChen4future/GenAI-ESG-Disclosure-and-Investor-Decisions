...
def _build_calibration_context(player):
    cached = player.participant.vars.get('calibration_context_y1')
    if cached:
        return cached

    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    static_dir = os.path.join(project_root, '_static', 'investor_y1')
    ...
    for item in CALIBRATION_PDF_CONFIG:
        ...
        url_path = f"/static/investor_y1/{filename}"
    ...
    context = "\n\n".join(sections)
    player.participant.vars['calibration_context_y1'] = context
    return context
...
class C(BaseConstants):
    ...
    EXAMPLE_LOW = {
        ...
        "file_path": "investor_y1/calib_30.pdf",
    ...
    EXAMPLE_MED = {
        "score": 60,
        "label": "60分 - 运营合规 (风险控制)",
        "file_path": "investor_y1/calib_60.pdf",
    ...
    EXAMPLE_HIGH = {
        "score": 90,
        "label": "90分 - 战略融合 (深度赋能)",
        "file_path": "investor_y1/calib_90.pdf",
...
class Player(BasePlayer):
    ...
    quiz_2 = models.StringField(
        label='公司的董事会结构采用了“3+3+3”模式，这种模式具体是指哪三类董事各占3名？',
        choices=[
            ['A', 'A. 股东董事、独立董事、职工董事'],
            ['B', 'B. 股东董事、独立董事、执行董事'],
            ['C', 'C. 执行董事、非执行董事、独立董事'],
    ...
    quiz_3 = models.FloatField(
        label='本公司全年对外捐赠及公益总投入为多少万元？（请填写数字）'
    )
    ...
    def live_method(player, data):
        ...
        if data.get('type') == 'check_status':
            ...
            if others:
                ...
                is_ready = manager.participant.vars.get('manager_y1_done', False)
...
class InvestorReading(Page):
    ...
    def vars_for_template(player):
        ...
        if others:
            partner = others[0]
            # 1. 获取文本内容 (LaTeX源码)
            manager_text = partner.participant.vars.get('final_report_text', "Report content not found.")

            # 2. 获取参数以构建 PDF 路径
            l = partner.participant.vars.get('final_linguistic', 1)
            f = partner.participant.vars.get('final_format', 1)
            p = partner.participant.vars.get('final_proximity', 1)

            # 3. 构建 PDF URL
            manager_app_name = 'manager_y1'
...
class ComprehensionQuiz(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        if player.quiz_2 == 'B':
            quiz_pts += 1
        if player.quiz_3 is not None and 1000 <= float(player.quiz_3) <= 1500:
            quiz_pts += 1
        ...
        # ==========================================================
        # 【关键修改】存储 Investor 变量 (增加 _y1 后缀)
        # ==========================================================
        player.participant.vars['investpayoff_y1'] = float(inv_total)

        player.participant.vars['inv_accuracy_pay_y1'] = float(acc_pay)
        player.participant.vars['inv_quiz_pay_y1'] = float(q_pay)
        player.participant.vars['inv_true_val_y1'] = C.TRUE_COMPANY_VALUE
        player.participant.vars['inv_est_val_y1'] = player.estimated_score
        player.participant.vars['inv_report_open_count_y1'] = int(player.raw_pdf_open_count)
        player.participant.vars['inv_calibration_open_count_y1'] = int(player.calibration_open_count)
        player.participant.vars['inv_report_new_window_click_count_y1'] = int(player.report_new_window_click_count)
        player.participant.vars['inv_quick_prompt_click_count_y1'] = int(player.quick_prompt_click_count)
        player.participant.vars['inv_manual_prompt_send_count_y1'] = int(player.manual_prompt_send_count)
        player.participant.vars['inv_quick_prompt_send_count_y1'] = int(player.quick_prompt_send_count)
        player.participant.vars['inv_chat_send_count_y1'] = int(player.chat_send_count)
        player.participant.vars['inv_gpt_direct_score_prompt_count_y1'] = int(player.gpt_direct_score_prompt_count)
        player.participant.vars['inv_gpt_direct_score_extract_success_y1'] = bool(player.gpt_direct_score_extract_success)
        player.participant.vars['inv_gpt_direct_score_y1'] = player.field_maybe_none('gpt_direct_score')

        # ==========================================================
        # 【关键修改】存储 Manager 变量 (增加 _y1 后缀)
        # ==========================================================
        others = player.get_others_in_group()
        if others:
            manager = others[0]
            ...
            manager.participant.vars['investpayoff_y1'] = manager_game_payoff

            # 存储细节
            manager.participant.vars['mgr_score_y1'] = investor_rating
            manager.participant.vars['mgr_base_y1'] = float(C.MANAGER_BASE_PAY)
            manager.participant.vars['mgr_bonus_y1'] = float(C.MANAGER_RETURN_RATE * investor_rating)
