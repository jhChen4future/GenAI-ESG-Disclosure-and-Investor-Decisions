...
class C(BaseConstants):
    NAME_IN_URL = 'investor_svo'  # 如果是 manager 请改为 manager_svo
    PLAYERS_PER_GROUP = 2
    NUM_ROUNDS = 1
...
class SVO_TaskPage(Page):
    ...
    @staticmethod
    def is_displayed(player):
        return player.participant.vars.get('role') == 'Investor'  # 同上，需确认角色
...
class SVO_Results(Page):
    ...
    def vars_for_template(player):
        return {
            'selected_round': player.svo_selected_round,
            'earned_points': player.svo_earned_points,
            'money': player.svo_payoff_money
        }

page_sequence = [SVO_Introduction, SVO_TaskPage]