...
逻辑与 Y1 相同，但数据存储在 _y3 后缀变量中，且每轮重新随机匹配对手。
"""


class C(BaseConstants):
    NAME_IN_URL = 'manager_y3' # 【修改】App 名称
    PLAYERS_PER_GROUP = 2
    NUM_ROUNDS = 1
...
class Group(BaseGroup):
    # 存储最终生效的报告（数据库字段无需加后缀，因为这是 y3 独立的表）
...
class Player(BasePlayer):
    # --- 策略法决策字段 ---
    # 同样，数据库字段无需改名，因为这是 manager_y3 独立的数据表
...
class Manager(Page):
    ...
    def vars_for_template(player):
        player.participant.vars['manager_strategy_start_ts_y3'] = time.time()
        grid_items = []
    ...
    def before_next_page(player, timeout_happened):
        if player.participant.vars.get('role') == 'Manager':
            started_at = player.participant.vars.get('manager_strategy_start_ts_y3')
            if started_at is not None:
                player.strategy_form_seconds = max(0, time.time() - float(started_at))
                player.participant.vars['manager_strategy_form_seconds_y3'] = player.strategy_form_seconds
            ...
            # 5. 【核心修改】存入 participant.vars (使用 _y3 后缀)
            # 这样 Investor_y3 才能读到正确的数据，且不覆盖 y1
            player.participant.vars['final_report_text_y3'] = final_latex
            player.participant.vars['final_linguistic_y3'] = selected_l
            player.participant.vars['final_format_y3'] = selected_f
            player.participant.vars['final_proximity_y3'] = selected_p
            player.participant.vars['manager_y3_done'] = True
