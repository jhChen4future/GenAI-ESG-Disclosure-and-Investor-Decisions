...
def _build_calibration_context(player):
    cached = player.participant.vars.get('calibration_context_y3')
    if cached:
        return cached

    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    static_dir = os.path.join(project_root, '_static', 'investor_y3')
    ...
    for item in CALIBRATION_PDF_CONFIG:
        ...
        url_path = f"/static/investor_y3/{filename}"
    ...
    context = "\n\n".join(sections)
    player.participant.vars['calibration_context_y3'] = context
    return context
...
def _call_openai_compatible(messages):
    if not API_KEY:
        raise RuntimeError('BIANXIE_API_KEY is not set')
...
class C(BaseConstants):
    ...
    EXAMPLE_LOW = {
        ...
        "file_path": "investor_y3/calib_30.pdf",
    ...
    EXAMPLE_MED = {
        "score": 60,
        "label": "60分 - 运营合规 (风险控制)",
        "file_path": "investor_y3/calib_60.pdf",
    ...
    EXAMPLE_HIGH = {
        "score": 90,
        "label": "90分 - 战略融合 (深度赋能)",
        "file_path": "investor_y3/calib_90.pdf",
...
class Player(BasePlayer):
    ...
    quiz_1 = models.StringField(
        label='根据报告中的温室气体排放数据，2024年公司占比最高的排放类别是哪一项？',
        choices=[
            ['A', 'A. 范围1：直接排放（如汽油燃烧）'],
            ['B', 'B. 范围2：间接排放（如外购电力）'],
            ['C', 'C. 范围3：员工差旅排放'],
    ...
    quiz_2 = models.StringField(
        label='2024年公司在提升旅客体验和业务拓展方面取得了哪些具体成果？',
        choices=[
            ['A', 'A. 旅客吞吐量突破3,000万人次，核心门户机场排名进入全国前20位'],
            ['B', 'B. 实施了“一机双屏”模式，让旅客只需接受一次检查即可完成海关与安检联检'],
            ['C', 'C. 针对外籍旅客，主要在商业综合体提供移动支付绑定和电话卡购买服务'],
            ['D', 'D. 旗下机场群在2024年开通了首条前往新加坡的国际客运航线'],
        ],
        widget=widgets.RadioSelect
    )
    quiz_3 = models.FloatField(
        label='本公司全年累计减少碳排放CO2当量多少吨？（请填写数字）'
    )
    ...
    def live_method(player, data):
        ...
        if data.get('type') == 'check_status':
            ...
            if others:
                ...
                is_ready = manager.participant.vars.get('manager_y3_done', False)
...
class InvestorReading(Page):
    ...
    def vars_for_template(player):
        ...
        if others:
            ...
            # 1. 获取文本内容 (LaTeX源码)
            # 【关键修改】必须加 _y3 后缀
            manager_text = partner.participant.vars.get('final_report_text_y3', "Report content not found.")

            # 2. 获取参数以构建 PDF 路径
            # 【关键修改】必须加 _y3 后缀
            l = partner.participant.vars.get('final_linguistic_y3', 1)
            f = partner.participant.vars.get('final_format_y3', 1)
            p = partner.participant.vars.get('final_proximity_y3', 1)

            # 3. 构建 PDF URL
            # 【关键修改】静态文件夹指向 manager_y3
            manager_app_name = 'manager_y3' 
...
class ComprehensionQuiz(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        if player.quiz_2 == 'B':
            quiz_pts += 1
        if player.quiz_3 is not None and 20000 <= float(player.quiz_3) <= 30000:
            quiz_pts += 1
        ...
        # ==========================================================
        # 【关键修改】存储 Investor 变量 (增加 _y3 后缀)
        # ==========================================================
        player.participant.vars['investpayoff_y3'] = float(inv_total)

        player.participant.vars['inv_accuracy_pay_y3'] = float(acc_pay)
        player.participant.vars['inv_quiz_pay_y3'] = float(q_pay)
        player.participant.vars['inv_true_val_y3'] = C.TRUE_COMPANY_VALUE
        player.participant.vars['inv_est_val_y3'] = player.estimated_score
        player.participant.vars['inv_report_open_count_y3'] = int(player.raw_pdf_open_count)
        player.participant.vars['inv_calibration_open_count_y3'] = int(player.calibration_open_count)
        player.participant.vars['inv_report_new_window_click_count_y3'] = int(player.report_new_window_click_count)
        player.participant.vars['inv_quick_prompt_click_count_y3'] = int(player.quick_prompt_click_count)
        player.participant.vars['inv_manual_prompt_send_count_y3'] = int(player.manual_prompt_send_count)
        player.participant.vars['inv_quick_prompt_send_count_y3'] = int(player.quick_prompt_send_count)
        player.participant.vars['inv_chat_send_count_y3'] = int(player.chat_send_count)
        player.participant.vars['inv_gpt_direct_score_prompt_count_y3'] = int(player.gpt_direct_score_prompt_count)
        player.participant.vars['inv_gpt_direct_score_extract_success_y3'] = bool(player.gpt_direct_score_extract_success)
        player.participant.vars['inv_gpt_direct_score_y3'] = player.field_maybe_none('gpt_direct_score')

        # ==========================================================
        # 【关键修改】存储 Manager 变量 (增加 _y3 后缀)
        # ==========================================================
        others = player.get_others_in_group()
        if others:
            manager = others[0]
            ...
            manager.participant.vars['investpayoff_y3'] = manager_game_payoff

            # 存储细节
            manager.participant.vars['mgr_score_y3'] = investor_rating
            manager.participant.vars['mgr_base_y3'] = float(C.MANAGER_BASE_PAY)
            manager.participant.vars['mgr_bonus_y3'] = float(C.MANAGER_RETURN_RATE * investor_rating)
